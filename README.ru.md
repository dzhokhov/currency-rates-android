...
- **Версия 0.4.0 не ставится поверх 0.3.0.** Идентификатор приложения сменился с `com.dzhokhov.currencyrates` на `io.github.dzhokhov.quotes`, поэтому для системы это другое приложение: прежнее нужно удалить. Список валют теряется один раз — только при этом переходе.
...
Контрольная сумма подтверждает, что файл дошёл целым. Чтобы подтвердить, **кто его собрал**, проверьте сертификат подписи:

```bash
apksigner verify --print-certs currency-rates-<версия>.apk
```
...
**Два канала — две подписи.** Сборки отсюда подписаны ключом проекта с отпечатком выше. Когда приложение появится в Google Play, магазинная сборка будет подписана ключом Google (Play App Signing переподписывает загруженный пакет), поэтому **её отпечаток будет другим** — он будет опубликован рядом, как только появится. Следствие, которое стоит знать до выбора канала: сборки разных каналов не обновляют друг друга и не стоят на устройстве одновременно, поэтому переход означает удаление и потерю сохранённого списка один раз.

## Сборка из исходников

Нужны JDK 17 и Android SDK с платформой 36 и build-tools 36.0.0; версия Gradle (8.11.1) закреплена обёрткой.
